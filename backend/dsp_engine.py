...
MIN_VOICED_FRAMES = 3
SPEAKER_N_FFT = 512
SPEAKER_MEL_BANDS = 26
SPEAKER_CEPSTRAL_COEFFICIENTS = 13
...
def _mel_filterbank(sample_rate: int) -> np.ndarray:
    """Build a compact mel filterbank for the speaker voiceprint path."""
    minimum_mel = 2595.0 * np.log10(1.0 + 80.0 / 700.0)
    maximum_mel = 2595.0 * np.log10(1.0 + 7_600.0 / 700.0)
    mel_points = np.linspace(minimum_mel, maximum_mel, SPEAKER_MEL_BANDS + 2)
    frequencies = 700.0 * (10.0 ** (mel_points / 2595.0) - 1.0)
    bins = np.floor((SPEAKER_N_FFT + 1) * frequencies / sample_rate).astype(int)
    bins = np.clip(bins, 0, SPEAKER_N_FFT // 2)

    filters = np.zeros((SPEAKER_MEL_BANDS, SPEAKER_N_FFT // 2 + 1))
    for index in range(1, SPEAKER_MEL_BANDS + 1):
        left, center, right = bins[index - 1 : index + 2]
        if center <= left:
            center = min(left + 1, SPEAKER_N_FFT // 2)
        if right <= center:
            right = min(center + 1, SPEAKER_N_FFT // 2)
        if center > left:
...
def _unit_vector(values: np.ndarray) -> np.ndarray:
    ...
    if norm <= 1e-12:
        raise AudioProcessingError("Speaker features have insufficient variation.")
    return values / norm
...
def extract_speaker_embedding(
    ...
    """Create a normalized MFCC-statistics voiceprint for speaker comparison.

    This is an additional prototype similarity signal. It is deliberately kept
    separate from the public formant contract and is not a production speaker
    recognition model.
    """
    decoded_rate, audio = _decode_audio(audio_bytes, content_type, sample_rate)
    voice = _resample(audio, decoded_rate, FORMANT_SAMPLE_RATE)
    voice = voice - float(np.mean(voice))
    frames = _frame_audio(voice)
    voiced = [
        frame
        for frame in frames
        if float(np.sqrt(np.mean(frame * frame))) >= MIN_AUDIO_RMS
        and _frame_periodicity(frame) >= MIN_FRAME_PERIODICITY
    ]
    if len(voiced) < MIN_VOICED_FRAMES:
        raise AudioProcessingError("Not enough voiced audio for speaker comparison.")

    framed = np.asarray(voiced) * np.hamming(voiced[0].size)
    power = np.abs(np.fft.rfft(framed, n=SPEAKER_N_FFT, axis=1)) ** 2
    mel_energy = power @ _mel_filterbank(FORMANT_SAMPLE_RATE).T
    log_mel = np.log(np.maximum(mel_energy, 1e-12))
    cepstra = dct(log_mel, type=2, norm="ortho", axis=1)[
        :, 1:SPEAKER_CEPSTRAL_COEFFICIENTS
    ]
    deltas = np.diff(cepstra, axis=0)
    blocks = (
        np.mean(cepstra, axis=0),
        np.std(cepstra, axis=0),
        np.mean(deltas, axis=0),
        np.std(deltas, axis=0),
    )
    normalized_blocks = []
    for block in blocks:
        values = np.asarray(block)
        norm = float(np.linalg.norm(values))
        normalized_blocks.append(values / norm if norm > 1e-12 else np.zeros_like(values))
    embedding = _unit_vector(np.concatenate(normalized_blocks))
